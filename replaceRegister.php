 ...
 */
    class replaceRegister extends PluginBase
    {
        ...
        /**
         * Get the setting for this survey, else global else default
         * @param string $setting the setting name
         * @param int $iSurveyId the survey id
         * @param mixed $default The default value to use when not was set by any system
         * @return mixed
        */
        public function getSurveySetting($setting,$iSurveyId,$default=null)
        {
            $value=$this->get($setting, 'Survey', $iSurveyId,$this->get($setting));
            if(null===$value)
            {
                $value= (isset($this->settings[$setting]['default'])) ? $this->settings[$setting]['default'] : $default;
            }
            return $value;
        }
        public function beforeSurveySettings()
        {
            ...
            $aSettings=array(
                'bUse' => array(
                    ...
                    'current' => $this->getSurveySetting('bUse', $iSurveyId),
                ),
                ...
                'bShowTokenForm' => array(
                    ...
                    'current' => $this->getSurveySetting('bShowTokenForm', $iSurveyId),
                ),
                'bConfirmEmail' => array(
                    'type' => 'select',
                    'options'=>array(
                ...
                'sEmailTemplate' => array(
                    ...
                    'tab'=>'tokens', // Setting no used yet
                    'label' => 'Email template to use if email is already in token table',
                    'current' => $this->getSurveySetting('sEmailTemplate', $iSurveyId),
                ),
                'bFirstnameMandatroy'=>array(
                    ...
                    'current' => $this->getSurveySetting('bFirstnameMandatroy', $iSurveyId,0),
                ),
                'bLastnameMandatroy'=>array(
                    ...
                    'current' => $this->getSurveySetting('bLastnameMandatroy', $iSurveyId,0),
                ),
                'bEmailMandatroy'=>array(
                    ...
                    'current' => $this->getSurveySetting('bEmailMandatroy', $iSurveyId,1),
                ),
                ...
                'sPrefillWithDate' => array(
                    ...
                    'current' => $this->getSurveySetting('sPrefillWithDate', $iSurveyId),
                ),
                'sPrefillDateSystem' => array(
                    ...
                    'current' => $this->getSurveySetting('sPrefillDateSystem', $iSurveyId, 'school'),
                ),
                'sPrefillDateDuplicate' => array(
                    ...
                    'current' => $this->getSurveySetting('sPrefillDateSystem', $iSurveyId, 1),
                ),
            );
            // Remove invalid
            ...
            $aSettings['sDescriptionText']=array(
                ...
                'current'=>$this->getSurveySetting('sDescriptionText', $iSurveyId, ""),
            );
            $aSettings['sErrorCompleted']=array(
                ...
                'current'=>$this->getSurveySetting('sErrorCompleted', $iSurveyId, ""),
            );
            $aSettings['sErrorOptoput']=array(
                ...
                'current'=>$this->getSurveySetting('sErrorOptoput', $iSurveyId, ""),
            );
            $aSettings['sErrorBounced']=array(
                ...
                'current'=>$this->getSurveySetting('sErrorBounced', $iSurveyId, ""),
            );
            $aSettings['sErrorNoemail']=array(
                ...
                'current'=>$this->getSurveySetting('sErrorNoemail', $iSurveyId, ""),
            );
            $aSettings['sSuccessRegistred']=array(
                ...
                'current'=>$this->getSurveySetting('sSuccessRegistred', $iSurveyId, ""),
            );
            $aSettings['sDescriptionToken']=array(
                ...
                'current'=>$this->getSurveySetting('sDescriptionToken', $iSurveyId, ""),
            );
            if(empty($aTokenInformations['aUsableAttribute']))
            {
                $aSettings['sAttributeDate']=array(
                    'type'=>'info',
                    'label'=>gt("Attribute with date"),
                    'content'=>CHtml::textField("","No valid attribute",array("disabled"=>true))
                );
            }
        ...
        public function beforeSurveyPage()
        {
            ...
            if(!tableExists("{{tokens_{$iSurveyId}}}"))
                return;
            if(!$this->getSurveySetting('bUse', $iSurveyId))
                return;
            ...
            if($iSurveyId && !$oToken)
            {
                ...
                {
                    // Fill parameters
                    $bShowTokenForm=$this->getSurveySetting('bShowTokenForm', $iSurveyId);
                    $bConfirmEmail=$this->getSurveySetting('bConfirmEmail', $iSurveyId);
                    ...
                    $sDateAttribute=$this->getSurveySetting('sAttributeDate', $iSurveyId);
                    if($sDateAttribute)
                    {
                        $sDateMin=$this->getSurveySetting('sAttributeDateMin', $iSurveyId, "100 years ago");
                        $sDateMax=$this->getSurveySetting('sAttributeDateMax', $iSurveyId,"now");
                    ...
                    $sHtmlRegistermessage2=trim($this->getSurveySetting('sDescriptionText', $iSurveyId,""));
                    ...
                    if($sAction=='register' && !is_null($sR_email) && Yii::app()->request->getPost('changelang')!='changelang')
                    {
                        ...
                        elseif($this->getSurveySetting('bEmailMandatroy', $iSurveyId,1))
                        {
                            $aRegisterError[]= sprintf(gT("%s cannot be left empty").".", gt("Email address"));
                        }
                        // Add firstname and lmast name if settings ask it
                        if($this->getSurveySetting('bFirstnameMandatroy', $iSurveyId,0) && $sR_firstname=="")
                                $aRegisterError[]= sprintf(gT("%s cannot be left empty").".", gt("First name"));
                        if($this->getSurveySetting('bLastnameMandatroy', $iSurveyId,0) && $sR_firstname=="")
                        ...
                        if(count($aRegisterError)==0)
                        {
                            ...
                            if($sPrefillWithDate=$this->getSurveySetting('sPrefillWithDate', $iSurveyId))
                            {
                                // @todo Validate if exist
                                $now=dateShift(date("Y-m-d H:i:s"), "Y-m-d H:i", Yii::app()->getConfig('timeadjust'));
                                switch($this->getSurveySetting('sPrefillDateSystem', $iSurveyId,'school'))
                                {
                                    case 'year':
                                ...
                                if($this->getSurveySetting('sPrefillDateDuplicate', $iSurveyId,1))
                                {
                            ...
                            if ($oToken)
                            {
                                if($oToken->email!="")
                                {
                                    ...
                                    {
                                        $sRegisterError=trim($this->getSurveySetting('sErrorCompleted', $iSurveyId));
                                    ...
                                    {
                                        $sRegisterError=trim($this->getSurveySetting('sErrorOptoput', $iSurveyId));
                                        if(empty($sRegisterError))
                                    ...
                                    {
                                        $sRegisterError=trim($this->getSurveySetting('sErrorBounced', $iSurveyId));
                                        if(empty($sRegisterError))
                                    ...
                                    {
                                        $iTokenId=$oToken->tid;
                                        $sEmailTemplate=$this->getSurveySetting('sEmailTemplate', $iSurveyId);
                                        if($sEmailTemplate=="none")
                                        {
                                            $sRegisterError=trim($this->getSurveySetting('sSuccessRegistred', $iSurveyId));
                                            if(empty($sRegisterError))
                                        ...
                                        else
                                        {
                                            ...
                                            $sInformation=trim($this->getSurveySetting('sSuccessRegistred', $iSurveyId));
                                ...
                                {
                                    $sRegisterError=trim($this->getSurveySetting('sErrorNoemail', $iSurveyId));
                                    if(empty($sRegisterError))
                    ...
                    if($aMail && $oToken)
                    {
                        ...
                        if(isset($bNewToken) && $this->getSurveySetting('bRedirectWithout', $iSurveyId,0))
                        {
                    ...
                    if($bShowForm || count($aRegisterError))
                    {
                        ...
                        $bIsRequired=(bool) ($this->getSurveySetting('bFirstnameMandatroy', $iSurveyId,0));
                        $sHtmlRegisterform.=  "<tr".( $bIsRequired ? " class='mandatory'" : '')."><th><label for='register_firstname'>".gT("First name") . "</label></th><td>".CHtml::textField('register_firstname',htmlentities($sR_firstname, ENT_QUOTES, 'UTF-8'),array('class'=>'text','required'=>$bIsRequired))."</td></tr>\n";
                        $bIsRequired=(bool) ($this->getSurveySetting('bLastnameMandatroy', $iSurveyId,0));
                        $sHtmlRegisterform.=  "<tr".( $bIsRequired ? " class='mandatory'" : '')."><th><label for='register_lastname'>".gT("Last name") . "</label></th><td>".CHtml::textField('register_lastname',htmlentities($sR_lastname, ENT_QUOTES, 'UTF-8'),array('class'=>'text','required'=>$bIsRequired))."</td></tr>\n";
                        // Extra attribute
                        foreach ($aSurveyInfo['attributedescriptions'] as $field => $aAttribute)
                        {
                            if (!empty($aAttribute['show_register']) && $aAttribute['show_register'] == 'Y')
                        ...
                        $bIsRequired=(bool) ($this->getSurveySetting('bEmailMandatroy', $iSurveyId,1));
                        $sHtmlRegisterform.=  "<tr".( $bIsRequired ? " class='mandatory'" : '')."><th><label for='register_email'>".gT("Email address") . "</label></th><td>".CHtml::textField('register_email',htmlentities($sR_email, ENT_QUOTES, 'UTF-8'),array('class'=>'text','required'=>$bIsRequired))."</td></tr>\n";
                        if($bConfirmEmail)
                            $sHtmlRegisterform.=  "<tr".( $bIsRequired ? " class='mandatory'" : '')."><th><label for='register_emailcontrol'>".gT("Confirmation") . "</label></th><td>".CHtml::textField('register_emailcontrol',htmlentities($sR_emailcontrol, ENT_QUOTES, 'UTF-8'),array('class'=>'text','required'=>$bIsRequired))."</td></tr>\n";
                        ...
                        if($bShowTokenForm)
                        {
                            ...
                            $sTokenMessage=trim($this->getSurveySetting('sDescriptionToken', $iSurveyId));
